...
from launch.actions import LogInfo, TimerAction, RegisterEventHandler, ExecuteProcess
...
def generate_launch_description():
    ...
    start_robot_ik = ExecuteProcess (
        cmd=['ros2', 'launch', 'robot_ik', 'robot_ik.launch.py'],
        name='robot_ik',
        output='screen'
    )
    ...
    close_evt4 = RegisterEventHandler(
        event_handler=OnProcessStart(
            ...
            on_start=[TimerAction(period=1.0, actions=[start_robot_ik, log_robot_driver_started])],
        )
    )
